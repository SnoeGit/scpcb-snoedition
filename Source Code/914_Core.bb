	...
	Select item\ItemTemplate\TempName
			...
			Select Setting
					...
					Select Level
						...
						Case 5
							...
							Select SelectedDifficulty\OtherFactors
								...
								Case EXTREME
									;[Block]
									If Rand(0, ((MAXACHIEVEMENTS - 1) * 6) - ((CurrAchvAmount - 1) * 2)) = 0 Then
										it2.Items = CreateItem("Key Card Omni", "keyomni", x, y, z)
									ElseIf Rand(26) = 1
										it2.Items = CreateItem("Level 6 Key Card", "key6", x, y, z)
									Else
				...
				Case VERYFINE
					...
					Select SelectedDifficulty\OtherFactors
						...
						Case EXTREME
							;[Block]
							If Rand(0, ((MAXACHIEVEMENTS - 1) * 7) - ((CurrAchvAmount - 1) * 3)) = 0 Then
								it2.Items = CreateItem("Key Card Omni", "keyomni", x, y, z)
							ElseIf Rand(36) = 1
								it2.Items = CreateItem("Level 6 Key Card", "key6", x, y, z)
							Else
		...
		Case "superbat", "killbat"
			...
			Select Setting
				...
				Case ONETOONE
					;[Block]
					If SelectedItem\ItemTemplate\TempName = "superbat" Then
						it2.Items = CreateItem("Strange Battery", "killbat", x, y, z)
					Else
						it2.Items = CreateItem("999V Battery", "superbat", x, y, z)
					EndIf
		...
		Case "cup"
			...
			Select Setting
				...
				Case VERYFINE
					...
					If Rand(2) = 1 Then me\ExplosionTimer = 135.0
					msg\DeathMsg = "Did you think it would become a cup of reassurance? Nope."
					;[End Block]
			End Select	
			;[End Block]
		Case "paper"

		...
		Select e\EventID
								...
								Else
									...
									If e\room\NPC[2]\State <> 1.0 And (Not me\Terminated) Then
										If EntityZ(e\room\NPC[2]\Collider) < e\room\z - 1150.0 * RoomScale Then
											...
											me\BlinkTimer = -10.0
											PlaySound2(StoneDragSFX, Camera, n_I\Curr173\Collider)
											If EntityDistanceSquared(n_I\Curr173\Collider, me\Collider) < 6.5 And Abs(EntityY(me\Collider) - EntityY(n_I\Curr173\Collider)) < 1.0 Then
							...
							Else
								...
								If e\room\NPC[3]\State3 >= 70.0 * 8.0 And e\room\NPC[3]\State3 - fps\Factor[0] < 70.0 * 8.0 And e\room\NPC[3]\State = 7.0 Then
									...
									ElseIf e\room\NPC[3]\State2 = 3.0
										For i = 3 To 4
											If ChannelPlaying(e\room\NPC[i]\SoundCHN) Then StopChannel_Strict(e\room\NPC[i]\SoundCHN)
											FreeSound_Strict(e\room\NPC[i]\Sound)
										Next
							...
							Else
								If e\room\RoomDoors[2]\Open Then
									...
									If e\room\NPC[3]\State3 >= 70.0 * 8.0 And e\room\NPC[3]\State3 - fps\Factor[0] / Temp < 70.0 * 8.0 And e\room\NPC[3]\State = 9.0 Then
										If e\room\NPC[3]\State2 < 2.0 Then
											If ChannelPlaying(e\room\NPC[3]\SoundCHN) Then StopChannel_Strict(e\room\NPC[3]\SoundCHN)
						...
						ElseIf e\EventState < 20000.0
							...
							If e\EventState < 14100.0 Then
								...
								ElseIf e\EventState < 14065.0
									...
									If ChannelPlaying(e\room\NPC[0]\SoundCHN) Then StopChannel_Strict(e\room\NPC[0]\SoundCHN)
								...
								Else
									...
									FreeSound_Strict(e\room\NPC[2]\Sound)
				...
				If (Not n_I\Curr106\Contained) Then
					...
					ElseIf e\EventState = 1.0
						If PlayerRoom = e\room Then
							...
							If e\SoundCHN <> 0 Then StopChannel_Strict(e\SoundCHN)
			...
			Case e_cont1_035
				...
				If PlayerRoom = e\room Then
					...
					Else ; ~ SCP-035 has left
						...
						If e\EventState3 > 0.0 And I_714\Using = 1 And wi\HazmatSuit <> 4 And wi\GasMask <> 4 Then 
			...
			Case e_cont1_106
				...
				Else
					If (Not PlayerInReachableRoom()) Then
						...
						If e\SoundCHN3 <> 0 Then If ChannelPlaying(e\SoundCHN3) Then StopChannel_Strict(e\SoundCHN3)
							...
							Case 67.0
								...
								If DistanceSquared(EntityX(me\Collider), EntityX(e\room\Objects[0], True), EntityZ(me\Collider), EntityZ(e\room\Objects[0], True)) < 8.0 And (Not chs\NoTarget) Then
									If Rand(150) = 1 Then
										...
										InjurePlayer(Rnd(0.25, 0.55), 0.0, 300.0)
										...
										If me\Injuries => 4.5 Then Kill(True)
									EndIf
								EndIf
			...
			Case e_cont2_1123
				...
				If PlayerRoom = e\room Then
					...
					If e\EventState > 0.0 Then CanSave = False
					If e\EventState = 1.0 Then
				...
				If PlayerRoom = e\room Then
					If e\EventState = 0.0 Then
						If EntityDistanceSquared(e\room\Objects[0], me\Collider) < 0.64 And e\EventState3 = 0.0
							...
							EndIf
							If e\Sound <> 0 Then StopChannel_Strict(e\SoundCHN)
							...
							If Temp < PowTwo(130.0 * RoomScale) Then
								For r.Rooms = Each Rooms
									If r\RoomTemplate\Name = "room2_shaft" Then
										...
										me\BlinkTimer = -10.0 : me\LightBlink = 5.0 : me\BlurTimer = 1500.0
										If wi\NightVision > 0 Then opt\CameraFogFar = 20.0

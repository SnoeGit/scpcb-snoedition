...
Function CreateNPC.NPCs(NPCType%, x#, y#, z#)
	...
	Select NPCType
		...
		Case NPCType860_2
			...
			Tex = LoadTexture_Strict("GFX\npcs\scps\scp_860_2_eyes.png", 1 + 2, DeleteAllTextures)
			n\OBJ2 = CreateSprite()
		...
		Case NPCType939
			...
			For n2.NPCs = Each NPCs
				If n\NPCType = n2\NPCType And n <> n2 Then i = i + Rand(25)
			Next
			n\NVGName = "SCP-939-" + i
					...
					If n\Idle < 2 Then
						...
						If n\Idle = 0 Then
							...
							If (Not Move) Then
								...
								me\CurrCameraZoom = Max(me\CurrCameraZoom, (Sin(Float(MilliSecs()) / 20.0) + 1.0) * 15.0 * Max((3.5 - Sqr(Dist)) / 3.5, 0.0))								
							...
							Else
								...
								Else ; ~ Actively move towards the player
									...
									; ~ Tries to open doors
									If Rand(20 - (5 * AggressiveNPCs)) = 1 Then
									...
									If Temp Then
										...
										If Dist < 0.4225 Then
											...
											EndIf
										Else
											If I_268\Using < 2 Lor I_268\Timer =< 0.0 Then
					...
					If n\Idle = 0 And Spawn106 Then
							...
							Else
								...
								If Visible Then
									...
									If EntityInView(n\Collider, Camera) Then
										...
										me\BlurVolume = Max(Max(Min((4.0 - Sqr(Dist)) / 6.0, 0.9), 0.1), me\BlurVolume)
										me\CurrCameraZoom = Max(me\CurrCameraZoom, (Sin(Float(MilliSecs()) / 20.0) + 1.0) * 20.0 * Max((4.0 - Sqr(Dist)) / 4.0, 0.0))
						...
						Else ; ~ Idling outside the map
							...
							If (Not PlayerRoom\RoomTemplate\DisableDecals) Lor PlayerRoom\RoomTemplate\Name = "room2_bio" Lor (PlayerRoom\RoomTemplate\Name = "cont1_173" And EntityY(me\Collider) < (100.0) * RoomScale) Lor (PlayerRoom\RoomTemplate\Name = "cont2_012" And EntityY(me\Collider) > (0.0) * RoomScale) Then
								If PlayerRoom\RoomTemplate\Name <> "gate_a" And (I_268\Using < 2 Lor I_268\Timer =< 0.0) Then
									If SelectedDifficulty\AggressiveNPCs = VERY_AGGRESSIVE Then
										n\State = Max(n\State - (fps\Factor[0] * 2.0), 0.0)
									ElseIf PlayerRoom\RoomTemplate\Name = "cont1_035" Lor PlayerRoom\RoomTemplate\Name = "cont1_173" Lor PlayerRoom\RoomTemplate\Name = "room2_tesla_lcz" Lor PlayerRoom\RoomTemplate\Name = "room2_tesla_hcz" Lor PlayerRoom\RoomTemplate\Name = "room2_tesla_ez"
										n\State = Max(n\State - (fps\Factor[0] * (0.5 + (SelectedDifficulty\AggressiveNPCs * 0.25))), 0.0)
									Else
										n\State = Max(n\State - (fps\Factor[0] * (1.0 + (SelectedDifficulty\AggressiveNPCs * 0.5))), 0.0)
									EndIf
								EndIf
							EndIf
						EndIf
				...
				Else
					...
					Select n\State
						...
						Case 2.0 ; ~ Being active
							...
							If (Dist < PowTwo(HideDistance * 2.0)) And n\Idle = 0 And PlayerInReachableRoom(True) Then
								...
								If PlayerSeeAble = 1 Lor n\State2 > 0.0 And (Not chs\NoTarget) Then ; ~ Player is visible for SCP-049's sight
									If (I_268\Using < 2 Lor I_268\Timer =< 0) Then
														...
														If (Not n\Path[n\PathLocation - 1]\door\IsElevatorDoor > 0) Then
															If EntityDistanceSquared(n\Path[n\PathLocation - 1]\OBJ, n\Collider) > 0.09 Then
																If (n\Path[n\PathLocation - 1]\door\MTFClose) And (n\Path[n\PathLocation - 1]\door\OpenState = 180.0) And (n\Path[n\PathLocation - 1]\door\Buttons[0] <> 0 Lor n\Path[n\PathLocation - 1]\door\Buttons[1] <> 0) Then
																	OpenCloseDoor(n\Path[n\PathLocation - 1]\door, True)
																EndIf
															EndIf
														EndIf
													EndIf
												...
												If n\Path[n\PathLocation]\door <> Null Then
													If (Not n\Path[n\PathLocation]\door\IsElevatorDoor > 0) Then
														...
														ElseIf (n\Path[n\PathLocation]\door\Locked > 0 Lor n\Path[n\PathLocation]\door\KeyCard > 5 Lor n\Path[n\PathLocation]\door\Code <> "") And (Not n\Path[n\PathLocation]\door\Open) Then
															Temp = False
														Else
															If (Not n\Path[n\PathLocation]\door\Open) And (n\Path[n\PathLocation]\door\Buttons[0] <> 0 Lor n\Path[n\PathLocation]\door\Buttons[1] <> 0) Then OpenCloseDoor(n\Path[n\PathLocation]\door, True)
														EndIf
													EndIf
												EndIf
						...
						Case 5.0 ; ~ Going to surveillance room
							...
							UpdateSoundOrigin(n\SoundCHN2, Camera, n\OBJ, 10.0, 1.0, True, True)
							;[End Block]
					End Select
				EndIf
					...
					Select n\State
						...
						Case 3.0 ; ~ Player isn't visible, tries to find
							...
							If PlayerSeeAble = 1 And (Not chs\NoTarget) Then
								If (I_268\Using < 2 Lor I_268\Timer =< 0.0) Then n\State = 2.0
								If Dist < 0.5 Then
			...
			Case NPCTypeGuard
				...
				Select n\State
					...
					Case 12.0
						...
						UpdateSoundOrigin(n\SoundCHN, Camera, n\Collider, 20.0, 1.0, True, True)
						;[End Block]
					Case 13.0
						;[Block]
			...
			Case NPCTypeApache
				...
				Select n\State
					...
					Case 2.0, 3.0 ; ~ Attacks
						...
						If chs\NoTarget And I_268\Using > 1 And I_268\Timer > 0.0 And n\State = 2.0 Then n\State = 1.0
					...
					If Dist < PowTwo(HideDistance) Then
						Select n\State 
							...
							Case 1.0 ; ~ Idles
								...
								If Dist < 3.24 And (Not chs\NoTarget) And (I_268\Using < 2 Lor I_268\Timer =< 0.0) Then
					...
					If forest_event\EventState = 1.0 Then
						...
						Select n\State
							...
							Case 2.0 ; ~ Appears on the path and starts to walk towards the player
								...
								Else
										...
										If Dist < 64.0 Then
											If EntityInView(n\Collider, Camera) Then
												If Rand(8) = 1 Then
													...
													PlaySound2(LoadTempSound("SFX\SCP\860\Cancer" + Rand(2) + ".ogg"), Camera, n\Collider)
												EndIf
												n\State2 = 1.0
											EndIf										
										EndIf
									...
									If Dist < 20.25 Lor n\State3 > Rnd(200.0, 250.0) Then
										...
										If (Not chs\NoTarget) And (I_268\Using < 2 Lor I_268\Timer =< 0.0) Then
											n\State = 3.0
										Else
							...
							Case 3.0 ; ~ Runs towards the player and attacks
								...
								If chs\NoTarget Then
									If I_268\Using > 1 And I_268\Timer > 0.0 Then
							...
							Case 4.0 ; ~ Idles (not hidden)
								...
								If (Not chs\NoTarget) And (I_268\Using < 2 Lor I_268\Timer =< 0.0) Then n\State = 3.0
								;[End Block]
						End Select
			...
			Case NPCType939
				...
				If n\State < 66.0 Then 
					...
					If n\State < 3.0 And (Not chs\NoTarget) And (Not n\IgnorePlayer) Then
						...
						If Dist < PowTwo(opt\CameraFogFar * LightVolume) And EntityInView(n\Collider, Camera) And EntityVisible(Camera, n\Collider) Then GiveAchievement(Achv939)
						
						If Dist < 16.0 Then Dist = Dist - PowTwo((EntityVisible(me\Collider, n\Collider) + (EntityVisible(me\Collider, n\Collider) * 0.21)))
						If (PowTwo(me\SndVolume) > Dist And (I_268\Using < 2 Lor I_268\Timer =< 0.0)) Lor Dist < 2.25 Then
							...
							n\State = 3.0
						ElseIf PowTwo(me\SndVolume * 1.4) > Dist And (I_268\Using < 2 Lor I_268\Timer =< 0.0)
				...
				Select n\State
					...
					Case 1.0 ; ~ Staring at the player
						...
						If Dist < 1.0 + PowTwo(n\LastDist) Then
							If I_268\Using < 2 Lor I_268\Timer =< 0.0 Then n\State = Rand(2.0, 3.0)
						...
						Select n\State
							Case 0.0 ; ~ Idles
								...
								Else
									...
									If n\Frame > 213.0
										If Rand(3) = 1 And Dist < 16.0 And (I_268\Using < 2 Lor I_268\Timer =< 0.0) Then
									...
									If Dist < 4.0 And (I_268\Using < 2 Lor I_268\Timer =< 0.0) Lor Dist < 1.4 Then n\State = Rand(4)
								EndIf
							...
							Case 1.0, 2.0 ; ~ Echo
								...
								If (Not chs\NoTarget) Then
									...
									If n\State3 < 900.0 Then
										...
										If I_714\Using <> 3 And wi\GasMask <> 4 And wi\HazmatSuit <> 4 And Dist < 128.0 Then
							...
							Case 5.0, 6.0, 8.0 ; ~ Walking or chasing
								...
								Else
									...
									If n\State = 8.0 And Dist < 1.4 And (Not chs\NoTarget) Then
										If EntityVisible(n\Collider, me\Collider) And I_268\Using > 1 And I_268\Timer > 0.0 Then
									...
									; ~ Chasing the player
									If n\State = 8.0 And Dist < 1024.0 And (Not chs\NoTarget) And (I_268\Using < 2 Lor I_268\Timer =< 0.0) Then
										If n\PathTimer <= 0.0 Then
											n\PathStatus = FindPath(n, EntityX(me\Collider, True), EntityY(me\Collider, True), EntityZ(me\Collider, True))
											...
											For n2.NPCs = Each NPCs
												If n2 <> n Then
													If n2\NPCType = n\NPCType Then
														If n2\Target = Null And n2\PrevState = 0 Then
		...
		Else
			...
			Select Int(n\State) ; ~ What is this MTF doing
				Case 0.0 ; ~ Wandering around
					...
					If (I_268\Using > 0 And I_268\Timer > 0.0) And Dist > 1.4 Then Temp = False
				...
				Case 1.0 ; ~ Searching for player
					...
					If NPCSeesPlayer(n) = 1 Then
						...
						If Dist < 25.0 And (I_268\Using < 2 Lor I_268\Timer =< 0.0) Lor Dist < 1.4 Then
				...
				Case 10.0 ; ~ Trying to find player on the Gate A or Gate B
					...
					If (Not me\Terminated) Then
						...
						Else
							...
							If Dist < 4.0 And chs\NoTarget And (I_268\Using < 2 Lor I_268\Timer =< 0.0) Then
								AnimateNPC(n, 79.0, 310.0, 0.35)
							Else
								If n\PathStatus = 1 Then
									If n\Path[n\PathLocation] = Null Then
			...
			If chs\NoTarget And (I_268\Using < 2 Lor I_268\Timer =< 0.0) And n\State = 1.0 Then n\State = 0.0

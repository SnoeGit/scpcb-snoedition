					...
					Select n\State
						...
						Case 3.0 ; ~ Player isn't visible, tries to find
							...
							If PlayerSeeAble = 1 And (Not chs\NoTarget) Then
								If (I_268\Using = 0 Lor I_268\Timer =< 0.0) Then n\State = 2.0
								If Dist < 0.5 Then
			...
			Case NPCTypeApache
				...
				Select n\State
					...
					Case 2.0, 3.0 ; ~ Attacks
						...
						If chs\NoTarget And I_268\Using > 0 And I_268\Timer > 0.0 And n\State = 2.0 Then n\State = 1.0
					...
					If forest_event\EventState = 1.0 Then
						...
						Select n\State
							...
							Case 4.0 ; ~ Idles (not hidden)
								...
								If (Not chs\NoTarget) And (I_268\Using = 0 Lor I_268\Timer =< 0.0) Then n\State = 3.0
								;[End Block]
						End Select
			...
			Case NPCType939
				...
				If n\State < 66.0 Then 
					...
					If n\State < 3.0 And (Not chs\NoTarget) And (Not n\IgnorePlayer) Then
						...
						If (PowTwo(me\SndVolume) > Dist And (I_268\Using = 0 Lor I_268\Timer =< 0.0)) Lor Dist < 2.25 Then
							...
							n\State = 3.0
						ElseIf PowTwo(me\SndVolume * 1.4) > Dist And (I_268\Using = 0 Lor I_268\Timer =< 0.0)
		...
		Else
			...
			Select Int(n\State) ; ~ What is this MTF doing
				Case 0.0 ; ~ Wandering around
					...
					If (I_268\Using > 0 And I_268\Timer > 0.0) And Dist > 1.5 Then Temp = False
					...
					If n_I\Curr106\State <= 0.0 Then
						If NPCSeesNPC(n_I\Curr106, n) Lor EntityDistanceSquared(n\Collider, n_I\Curr106\Collider) < 9.0 Then
							If EntityVisible(n\Collider, n_I\Curr106\Collider) Then
								...
								LoadNPCSound(n, "SFX\Character\MTF\106\Spotted" + Rand(2) + ".ogg")
								PlayMTFSound(n\Sound, n)
							EndIf
						EndIf
					EndIf
					...
					If n_I\Curr049 <> Null Then
						If NPCSeesNPC(n_I\Curr049, n) Then
							If EntityVisible(n\Collider, n_I\Curr049\Collider)
								...
								LoadNPCSound(n, "SFX\Character\MTF\049\Spotted" + Rand(2) + ".ogg")
								PlayMTFSound(n\Sound, n)
							EndIf
						EndIf
					EndIf
						...
						If n2\NPCType = NPCType049_2 And (Not n2\IsDead)
							If NPCSeesNPC(n2, n) Then
								If EntityVisible(n\Collider, n2\Collider)
									...
									n\Target = n2
									n\Reload = 70.0 * 5.0
									LoadNPCSound(n, "SFX\Character\MTF\049_2\Spotted" + Rand(2) + ".ogg")
									PlayMTFSound(n\Sound, n)
									Exit
								EndIf
							EndIf
				...
				Case 10.0 ; ~ Trying to find player on the Gate A or Gate B
					...
					If (Not me\Terminated) Then
						...
						Else
							...
							If Dist < 4.0 And chs\NoTarget And (I_268\Using = 0 Lor I_268\Timer =< 0.0) Then
								AnimateNPC(n, 79.0, 310.0, 0.35)
							Else
								If n\PathStatus = 1 Then
									If n\Path[n\PathLocation] = Null Then 
			...
			If chs\NoTarget And (I_268\Using = 0 Lor I_268\Timer =< 0.0) And n\State = 1.0 Then n\State = 0.0

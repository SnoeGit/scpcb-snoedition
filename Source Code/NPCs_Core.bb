			...
			Case NPCType049
				...
				If n\Idle > 0.1 Then
					If PlayerRoom\RoomTemplate\Name <> "cont2_049" Then
						n\Idle = Max(n\Idle - (1 + (0.5 * SelectedDifficulty\AggressiveNPCs)) * fps\Factor[0], 0.1)
					EndIf
					n\DropSpeed = 0.0
					...
					If n\State2 <= 0.0 And n\State2 + fps\Factor[0] > 0.0 Then
						If n\MTFLeader = Null Then
							...
							If MTFCameraCheckTimer = 0.0 Then
								If Rand(15 - (4 * SelectedDifficulty\AggressiveNPCs)) = 1 ; ~ Maybe change this to another chance -- ENDSHN
...
Function TeleportCloser%(n.NPCs)
	...
	If closestWaypoint <> Null Then
		If n\InFacility <> 1 Lor SelectedDifficulty\AggressiveNPCs = VERY_AGGRESSIVE Then
			ShouldTeleport = True
